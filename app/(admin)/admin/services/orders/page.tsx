    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
          ...
          <p className="text-2xl font-black text-orange-600">{orders?.filter(o => o.status === 'pending').length || 0}</p>
        </div>
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
          <p className="text-xs font-bold text-slate-400 uppercase">Dalam Proses</p>
          <p className="text-2xl font-black text-purple-600">{orders?.filter(o => o.status === 'process').length || 0}</p>
        </div>
      </div>
